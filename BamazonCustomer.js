...
//create a mysql connection with query to select stock_Quantity  column based on ? or user's ID number input under itemID column
connection.query('SELECT stock_Quantity FROM products WHERE ?', {item_id: productID}, function(err, res, field){

//since res returns the queried result, which contains in as mysql array of [rowdatapacket {stock_Quantity : 7000}]
//res[0] will limit down to rowdatapacket{stock_Quantity : 7000}, and .(columnName) will only return as the number
...
//if else statement to check if the currenstock (stock_Quantity  value of mysql) of the user selected ID is less than buying number, if it
...
//runs the updateStock function to update the inventory in mysql for stock_Quantity  value
...
var updateStock = function(newStock, productID){
  ...
  connection.query('UPDATE products SET ? WHERE ?',[{stock_Quantity: newStock},{item_id: productID}], function(err, res, field){
    connection.query('SELECT stock_Quantity FROM products WHERE ?', {item_id: productID}, function(err, res){
    if(err)throw err;
    console.log("...................................")
    console.log("Purchased!");
    console.log("the stock leftover now has " + res[0].stock_Quantity);
  })
